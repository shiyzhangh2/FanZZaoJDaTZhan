...
from os import *
def main():
    root = Tk(className='第七关')
    root.geometry('475x475')
    root.resizable(0,0)

    l_AI = Label(text = '答案是 We have a new classroom!')
    l_tip2 = Label(text = '(请自行退出)')

    l_CH = Label(text = '中文解释是：我们有间新教室！')
    # e输入框显示字符串:***
    def _show_1():
        e.insert(0, 'We')
    ...
    l = Label(text='第七关：如果一个字母分成了三个按钮该怎么办?').pack()
    la_1 = Label(text='(不能输入 请倒着输入答案)').pack()

    e = Entry(root, textvariable=content)
    e.pack()

    b_show_1 = Button(root, text='We', command=_show_1)
    b_show_1.pack()

    b_show_2 = Button(root, text='av', command=_show_2)
    b_show_2.pack()

    b_show_3 = Button(root, text='h', command=_show_3)
    b_show_3.pack()

    b_show_4 = Button(root, text='e a new', command=_show_4)
    b_show_4.pack()

    b_show_5 = Button(root, text='cl', command=_show_5)
    b_show_5.pack()

    b_show_6 = Button(root, text='assroom!', command=_show_6)
    b_show_6.pack()
